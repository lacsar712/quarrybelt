...
// ChuteDrivePulse counts chute-side effects from FSM after hooks (acceptance).
var ChuteDrivePulse func()

func RegisterChuteDriveHook(chain *HookChain) {
	chain.OnAfter(func(ctx context.Context, from, to model.PlantState, event PlantEvent) error {
		_ = ctx
		_ = from
		_ = to
		_ = event
		if ChuteDrivePulse != nil {
			ChuteDrivePulse()
		}
		return nil
	})
