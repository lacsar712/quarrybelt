...
func (p *PressureModel) Compute(snap model.PlantSnapshot, firing bool) (float64, error) {
	if !firing {
		return p.cooldown(snap.Yard.SteamPressurePSI), nil
	}
	base := snap.Settings.TargetSteamPSI
	beltFactor := snap.Beltbank.BeltFlowTPH / math.Max(snap.Settings.BeltFlowTPH, 1)
	scaleFactor := snap.Scale.LevelPercent / snap.Settings.ScaleLevelSetpoint
	if scaleFactor <= 0 {
		scaleFactor = 0.5
	}
	pressure := base * beltFactor * math.Min(scaleFactor, 1.2)
...
func (p *PressureModel) DeviationFromSetpoint(snap model.PlantSnapshot) float64 {
	return snap.Yard.SteamPressurePSI - snap.Settings.TargetSteamPSI
}
